...
from src.core.dependencies import verify_request_signature
from src.core.security import generate_secure_key
from src.models import APIKeyCreate, APIKeyResponse, UserResponse
...
async def get_me(current_user: dict = Depends(verify_request_signature)):
    """获取当前登录用户信息"""
...
async def list_my_keys(current_user: dict = Depends(verify_request_signature)):
    """列出当前用户的所有 API Key"""
...
@router.post("/me/keys", response_model=APIKeyResponse)
async def create_my_key(
    key_data: APIKeyCreate, current_user: dict = Depends(verify_request_signature)
):
    """创建新的 API Key"""
    # 生成新 Key
...
async def revoke_my_key(key_id: int, current_user: dict = Depends(verify_request_signature)):
    """吊销自己的 API Key"""
