...
async def list_all_users(current_user: dict = Depends(require_superuser)):
    """列出所有用户（超级用户专用）"""
...
async def promote_user(user_id: int, current_user: dict = Depends(require_superuser)):
    """提升用户为超级用户（超级用户专用）"""
    # 检查用户是否存在
    user = await user_storage.get_user_by_id(user_id)
    if not user:
...
async def deactivate_user(
    user_id: int, current_user: dict = Depends(require_superuser)
):
    """吊销用户账号（超级用户专用）"""
    # 不能吊销自己
...
async def activate_user(user_id: int, current_user: dict = Depends(require_superuser)):
    """激活已吊销的用户账号（超级用户专用）"""
    user = await user_storage.get_user_by_id(user_id)
    if not user:
        raise HTTPException(
